...
struct WaterTopContentView: View {
        ...
        ZStack {
            if waterViewModel.editingWaterGoal {
                ...
                Group {
                    ...
                    WaterWaveComponentShape(progress: min(1, CGFloat(Double(waterViewModel.waterConsumedToday) / waterViewModel.waterGoal)), waveHeight: waterViewModel.disableWave ? 0.0 : 0.1, offset: startAnimation)
                        ...
                        .animation(.easeInOut, value: waterViewModel.waterConsumedToday)
                        .transition(.opacity)
                }
                .transition(.opacity)
            }
    ...
    private var waterWaveOverlay: some View {
        Text("\(waterViewModel.waterConsumedToday) of \(Int(waterViewModel.waterGoal)) \(waterViewModel.useMetric ? "ml" : "oz")")
            .bold()
